---
name: create-team-pr
description: Create consistent, review-ready GitHub pull requests for a team. Use when Codex needs to prepare, validate, draft, open, or update a PR; summarize what changed and why; document tests, security impact, screenshots, risks, and follow-ups; or enforce the repository PR template with GitHub CLI.
---
...
5. Run the narrowest relevant tests, lint, formatting checks, builds, and security checks supported by the repository.
6. Confirm no secrets, generated artifacts, debug code, unrelated changes, or accidental dependency updates are included.
7. Write the title and body using the required format below.
8. Create or update the PR with `gh`. Preserve the branch unless the user explicitly requests deletion.
9. Return the PR URL, checks performed, failures or skipped checks, and any reviewer attention areas.
...
- Never mark a check as passed unless it ran successfully.
- Label skipped checks with the reason.
...
Write the body to a temporary file to avoid shell-quoting errors, then create or update:
